...
unsafe partial class StbVorbis
{
	...
	public static void compute_samples(int mask, short* output, int num_c, float*[] data, int d_offset, int len)
	{
		...
		for (o = 0; o < len; o += 32)
		{
			...
			if (o + n > len)
				n = len - o;
			for (j = 0; j < num_c; ++j)
				if ((channelPosition[num_c, j] & mask) != 0)
					for (i = 0; i < n; ++i)
	...
	public static void compute_stereo_samples(short* output, int num_c, float*[] data, int d_offset, int len)
	{
		...
		for (o = 0; o < len; o += 32 >> 1)
		{
			...
			for (j = 0; j < num_c; ++j)
			{
				var m = channelPosition[num_c, j] & (2 | 4);
				if (m == (2 | 4))
					for (i = 0; i < n; ++i)
					{
									  ...
									  byte* do_not_decode)
	{
		...
		var part_read = (int) (n_read / r.partSize);
		...
		if (rtype == 2 && ch != 1)
		{
			...
			for (pass = 0; pass < 8; ++pass)
			{
				...
				if (ch == 2)
					...
					{
						var z = (int) (r.begin + pcount * r.partSize);
						var c_inter = z & 1;
						var p_inter = z >> 1;
						if (pass == 0)
						{
				...
				for (j = 0; j < ch; ++j)
					if (do_not_decode[j] == 0)
					{
						...
						if (b >= 0)
						{
							...
							var offset = (int) (r.begin + pcount * r.partSize);
							var n2 = (int) r.partSize;
							var book = f.codebooks + b;
	...
	public static int getn(stb_vorbis z, byte* data, int n)
	{
		if (1 != 0)
		{
			...
			CRuntime.MemCopy(data, z.stream, (ulong) n);
			z.stream += n;
			return 1;
		}
	}
	...
	public static int is_whole_packet_present(stb_vorbis f)
	{
		...
		for (; s == -1;)
		{
			...
			if (p + 26 >= f.stream_end)
				return error(f, STBVorbisError.VORBIS_need_more_data);
			if (CRuntime.MemCompare(p, ogg_page_header, 4) != 0)
				return error(f, STBVorbisError.VORBIS_invalid_stream);
			if (p[4] != 0)
				return error(f, STBVorbisError.VORBIS_invalid_stream);
			if (first != 0)
	...
	public static int start_decoder(stb_vorbis f)
	{
		...
		for (i = 0; i < f.codebook_count; ++i)
		{
			...
			if (c->sparse != 0 && total >= c->entries >> 2)
			{
				...
				if (c->codeword_lengths == null)
					return error(f, STBVorbisError.VORBIS_outofmem);
				CRuntime.MemCopy(c->codeword_lengths, lengths, (ulong) c->entries);
		...
		for (i = 0; i < f.residue_count; ++i)
		{
			...
			r.residueBooks = new short[r.classifications, 8];
			for (j = 0; j < r.classifications; ++j)
			for (k = 0; k < 8; ++k)
				if ((residue_cascade[j] & (1 << k)) != 0)
				{
					r.residueBooks[j, k] = (short) get_bits(f, 8);
					if (r.residueBooks[j, k] >= f.codebook_count)
						return error(f, STBVorbisError.VORBIS_invalid_setup);
				}
				else
				{
	...
	public static int stb_vorbis_get_samples_float(stb_vorbis f, int channels, float** buffer, int num_samples)
	{
		...
		while (n < num_samples)
		{
			...
			{
				for (i = 0; i < z; ++i)
					CRuntime.MemCopy(buffer[i] + n, f.channel_buffers[i] + f.channel_buffer_start,
												...
												int right_start, int right_end, int* p_left)
	{
		...
		CRuntime.MemCopy(really_zero_channel, zero_channel, (ulong) (sizeof(int) * f.channels));
	...
	public static int vorbis_search_for_page_pushdata(stb_vorbis f, byte* data, int data_len)
	{
		...
		if (f.page_crc_tests < 4)
		{
			...
			for (i = 0; i < data_len; ++i)
				if (data[i] == 0x4f)
					if (0 == CRuntime.MemCompare(data + i, ogg_page_header, 4))
					{
						var j = 0;
						var len = 0;
						uint crc = 0;
	...
	{
		return CRuntime.MemCompare(data, vorbis_validate_vorbis, 6) == 0 ? 1 : 0;
	}
}
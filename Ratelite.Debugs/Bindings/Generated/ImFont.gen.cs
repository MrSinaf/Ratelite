...
public unsafe partial struct ImFontPtr
{
	...
	public Vector2 CalcTextSizeA(float size, float max_width, float wrap_width, ReadOnlySpan<char> text_begin)
	{
		...
		if (text_begin_byteCount > Utility.STACK_ALLOCATION_SIZE_LIMIT)
		{
			native_text_begin = Utility.Allocate(text_begin_byteCount + 1);
		}
		else
		{
		...
		var native_text_begin_offset = Utility.GetUtf8(text_begin, native_text_begin, text_begin_byteCount);
		...
		if (text_begin_byteCount > Utility.STACK_ALLOCATION_SIZE_LIMIT)
		{
			Utility.Free(native_text_begin);
		}
		return __retval;
	}
#endif
	...
	public string CalcWordWrapPositionA(float scale, ReadOnlySpan<char> text, float wrap_width)
	{
		...
		if (!text.IsEmpty)
		{
			...
			var native_text_offset = Utility.GetUtf8(text, native_text, text_byteCount);
		...
		return Utility.StringFromPtr(ret);
	}
#endif

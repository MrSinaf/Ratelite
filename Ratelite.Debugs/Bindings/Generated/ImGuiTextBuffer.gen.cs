    ...
    public unsafe partial struct ImGuiTextBufferPtr
    {
        ...
        public void append(ReadOnlySpan<char> str)
        {
            ...
            if (!str.IsEmpty)
            {
                ...
                if (str_byteCount > Utility.STACK_ALLOCATION_SIZE_LIMIT)
                {
                    native_str = Utility.Allocate(str_byteCount + 1);
                }
                else
                {
                ...
                int native_str_offset = Utility.GetUtf8(str, native_str, str_byteCount);
        ...
        public void appendf(ref ImGuiTextBuffer buffer, ReadOnlySpan<char> fmt)
        {
            ...
            if (!fmt.IsEmpty)
            {
                ...
                if (fmt_byteCount > Utility.STACK_ALLOCATION_SIZE_LIMIT)
                {
                    native_fmt = Utility.Allocate(fmt_byteCount + 1);
                }
                else
                {
                ...
                int native_fmt_offset = Utility.GetUtf8(fmt, native_fmt, fmt_byteCount);

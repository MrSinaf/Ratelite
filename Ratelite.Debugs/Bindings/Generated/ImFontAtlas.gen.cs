    ...
    public unsafe partial struct ImFontAtlasPtr
    {
        ...
        public ImFontPtr AddFontFromFileTTF(ReadOnlySpan<char> filename, float size_pixels)
        {
            ...
            if (!filename.IsEmpty)
            {
                ...
                if (filename_byteCount > Utility.STACK_ALLOCATION_SIZE_LIMIT)
                {
                    native_filename = Utility.Allocate(filename_byteCount + 1);
                }
                else
                {
                ...
                int native_filename_offset = Utility.GetUtf8(filename, native_filename, filename_byteCount);
        ...
        public ImFontPtr AddFontFromMemoryCompressedBase85TTF(ReadOnlySpan<char> compressed_font_data_base85, float size_pixels)
        {
            ...
            if (!compressed_font_data_base85.IsEmpty)
            {
                ...
                if (compressed_font_data_base85_byteCount > Utility.STACK_ALLOCATION_SIZE_LIMIT)
                {
                    native_compressed_font_data_base85 = Utility.Allocate(compressed_font_data_base85_byteCount + 1);
                }
                else
                {
                ...
                int native_compressed_font_data_base85_offset = Utility.GetUtf8(compressed_font_data_base85, native_compressed_font_data_base85, compressed_font_data_base85_byteCount);
            ...
            if (compressed_font_data_base85_byteCount > Utility.STACK_ALLOCATION_SIZE_LIMIT)
            {
                Utility.Free(native_compressed_font_data_base85);
            }
            return new ImFontPtr(ret);
        }
#endif

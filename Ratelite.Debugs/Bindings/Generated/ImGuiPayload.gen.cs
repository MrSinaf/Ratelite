    ...
    public unsafe partial struct ImGuiPayloadPtr
    {
        ...
        public bool IsDataType(ReadOnlySpan<char> type)
        {
            ...
            if (!type.IsEmpty)
            {
                ...
                if (type_byteCount > Utility.STACK_ALLOCATION_SIZE_LIMIT)
                {
                    native_type = Utility.Allocate(type_byteCount + 1);
                }
                else
                {
                ...
                int native_type_offset = Utility.GetUtf8(type, native_type, type_byteCount);

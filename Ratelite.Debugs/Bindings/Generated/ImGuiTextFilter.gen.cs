    ...
    public unsafe partial struct ImGuiTextFilterPtr
    {
        ...
        public bool Draw()
        {
            ...
            int label_byteCount = 0;
                ...
                if (label_byteCount > Utility.STACK_ALLOCATION_SIZE_LIMIT)
                {
                    native_label = Utility.Allocate(label_byteCount + 1);
                }
                else
                {
                ...
                int native_label_offset = Utility.GetUtf8("Filter(inc,-exc)", native_label, label_byteCount);
                native_label[native_label_offset] = 0;
            float width = 0.0f;
            byte ret = ImGuiNative.ImGuiTextFilter_Draw((ImGuiTextFilter*)(NativePtr), native_label, width);
            if (label_byteCount > Utility.STACK_ALLOCATION_SIZE_LIMIT)
            {
                Utility.Free(native_label);
            }
            return ret != 0;
        }
        ...
        public bool Draw(ReadOnlySpan<char> label)
        {
            ...
            if (!label.IsEmpty)
            {
                ...
                int native_label_offset = Utility.GetUtf8(label, native_label, label_byteCount);
                native_label[native_label_offset] = 0;
            }
        ...
        public bool PassFilter(ReadOnlySpan<char> text)
        {
            ...
            if (!text.IsEmpty)
            {
                ...
                if (text_byteCount > Utility.STACK_ALLOCATION_SIZE_LIMIT)
                {
                    native_text = Utility.Allocate(text_byteCount + 1);
                }
                else
                {
                ...
                int native_text_offset = Utility.GetUtf8(text, native_text, text_byteCount);

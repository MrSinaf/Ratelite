...
public struct Vector2Int(int x, int y) : IEquatable<Vector2Int>, ILerpable<Vector2Int>
{
	...
	public static Vector2Int max => new (int.MaxValue);
	public static Vector2Int min => new (int.MinValue);
	
	public int x { get; set; } = x;
	public int y { get; set; } = y;
	
	[JsonIgnore] public float length => MathF.Sqrt(x * x + y * y);
	[JsonIgnore] public int lengthSquared => x * x + y * y;
	[JsonIgnore] public int maxValue => (int)MathF.Max(x, y);
	[JsonIgnore] public int minValue => (int)MathF.Min(x, y);
	
	[JsonIgnore] public Vector2 aspect
	{
		get
		{
	...
	public bool IsOutsideBounds(Vector2Int min, Vector2Int max)
		=> x < min.x || x > max.x || y < min.y || y > max.y;
	
	public bool IsInsideBounds(Vector2Int min, Vector2Int max)
		=> x >= min.x && x <= max.x && y >= min.y && y <= max.y;
	...
	bool IEquatable<Vector2Int>.Equals(Vector2Int other) => x == other.x && y == other.y;
	...
	public static Vector2 operator /(Vector2Int a, float scalar)
		=> new (a.x / scalar, a.y / scalar);
	...
	public static Vector2 operator *(float scalar, Vector2Int a)
		=> new (a.x * scalar, a.y * scalar);

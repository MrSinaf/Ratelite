...
public struct Vector3(float x, float y, float z) : IEquatable<Vector3>, ILerpable<Vector3>
{
	...
	public static Vector3 max => new (float.MaxValue);
	public static Vector3 min => new (float.MinValue);
	
	public float x { get; set; } = x;
	public float y { get; set; } = y;
	public float z { get; set; } = z;
	
	[JsonIgnore] public float lengthSquared => x * x + y * y + z * z;
	[JsonIgnore] public float length => MathF.Sqrt(lengthSquared);
	[JsonIgnore] public float maxValue => MathF.Max(x, MathF.Max(y, z));
	[JsonIgnore] public float minValue => MathF.Min(x, MathF.Min(y, z));
	
	[JsonIgnore] public Vector3 normalized
	{
		get
		{
			var l = length;
	...
	{
		RoundingMode.Round => new Vector3Int(
			(int)MathF.Round(x),
			(int)MathF.Round(y),
			(int)MathF.Round(z)
		),
		RoundingMode.Floor => new Vector3Int(
			(int)MathF.Floor(x),
			(int)MathF.Floor(y),
			(int)MathF.Floor(z)
		),
		RoundingMode.Ceiling => new Vector3Int(
			(int)MathF.Ceiling(x),
			(int)MathF.Ceiling(y),
			(int)MathF.Ceiling(z)
		),
	...
	public static implicit operator Vector3(Vector3Int value) => new (value.x, value.y, value.z);
	
	public static Vector3 operator /(Vector3 a, float scalar)
		=> new (a.x / scalar, a.y / scalar, a.z / scalar);
	
	public static Vector3 operator /(float scalar, Vector3 a)
		=> new (scalar / a.x, scalar / a.y, scalar / a.z);

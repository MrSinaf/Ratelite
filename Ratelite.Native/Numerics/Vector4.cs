...
public struct Vector4(float x, float y, float z, float w) : IEquatable<Vector4>, ILerpable<Vector4>
{
	...
	{
		RoundingMode.Round => new Vector4Int(
			(int)MathF.Round(x),
			(int)MathF.Round(y),
			(int)MathF.Round(z),
			(int)MathF.Round(w)
		),
		RoundingMode.Floor => new Vector4Int(
			(int)MathF.Floor(x),
			(int)MathF.Floor(y),
			(int)MathF.Floor(z),
			(int)MathF.Floor(w)
		),
		RoundingMode.Ceiling => new Vector4Int(
			(int)MathF.Ceiling(x),
			(int)MathF.Ceiling(y),
			(int)MathF.Ceiling(z),
			(int)MathF.Ceiling(w)
	...
	public bool IsOutsideBounds(Vector4 min, Vector4 max)
		=> x < min.x || x > max.x || y < min.y || y > max.y ||
		   z < min.z || z > max.z || w < min.w || w > max.w;
	
	public bool IsInsideBounds(Vector4 min, Vector4 max)
		=> x >= min.x && x <= max.x && y >= min.y && y <= max.y &&
		   z >= min.z && z <= max.z && w >= min.w && w <= max.w;

...
/// <summary>
/// Représente une couleur RGBA (≧◡≦)~. Permet de manipuler les couleurs au format binaire,
/// hexadécimal, ou via <see cref="ILerpable{Color}"/>.
/// </summary>
/// <remarks>
/// Par défaut, l'alpha est à 255 (opaque), mais tu peux le baisser si tu aimes les choses
/// transparentes~
/// </remarks>
[StructLayout(LayoutKind.Sequential)]
public struct Color(byte r, byte g, byte b, byte a = byte.MaxValue)
		: IEquatable<Color>, ILerpable<Color>
{
	...
	public readonly (float r, float g, float b, float a) ToFloats()
	...
	public static ReadOnlySpan<Color> AsColors(ReadOnlySpan<byte> bytes)
		=> bytes.Length % 4 != 0
				? throw new ArgumentException(
					"Byte array length must be a multiple of 4. o((>ω< ))o"
				)
				: MemoryMarshal.Cast<byte, Color>(bytes);
	
	public static ReadOnlySpan<byte> AsBytes(Color[] colors)
		=> MemoryMarshal.Cast<Color, byte>(colors);

...
public struct Region(Vector2 position00, Vector2 position11) : ILerpable<Region>, IEquatable<Region>
{
	...
	public Region(float x, float y, float z, float w) :
			this(new Vector2(x, y), new Vector2(z, w)) { }
	...
	public static implicit operator Region(RegionInt value)
		=> new (value.position00, value.position11);
	
	public static bool operator ==(Region a, Region b) => a.Equals(b);
	public static bool operator !=(Region a, Region b) => !a.Equals(b);
	
	public static Region operator *(Region m, float scalar) => new (
		m.position00 * scalar,
		m.position11 * scalar
	);

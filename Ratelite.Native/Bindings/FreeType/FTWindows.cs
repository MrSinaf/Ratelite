...
namespace Ratelite.Bindings
{
    internal unsafe class FTWindows : IFT
    {
        ...
        {
            return FTWindowsNative.FT_Init_FreeType(alibrary);
        }
        ...
        {
            return FTWindowsNative.FT_Attach_File(face, filepathname);
        }

        public FT_Err FT_Attach_Stream(FT_FaceRec_* face, FT_Open_Args_* parameters)
        {
            return FTWindowsNative.FT_Attach_Stream(face, parameters);
        }

        public FT_Err FT_Reference_Face(FT_FaceRec_* face)
        {
            return FTWindowsNative.FT_Reference_Face(face);
        }

        public FT_Err FT_Done_Face(FT_FaceRec_* face)
        {
            return FTWindowsNative.FT_Done_Face(face);
        }

        public FT_Err FT_Select_Size(FT_FaceRec_* face, int strike_index)
        {
            return FTWindowsNative.FT_Select_Size(face, strike_index);
        }
        ...
        {
            return FTWindowsNative.FT_Select_Charmap(face, encoding);
        }

        public FT_Err FT_Set_Charmap(FT_FaceRec_* face, FT_CharMapRec_* charmap)
        {
            return FTWindowsNative.FT_Set_Charmap(face, charmap);
        }
        ...
        {
            return FTWindowsNative.FT_Face_Properties(face, num_properties, properties);
        }

        public uint FT_Get_Name_Index(FT_FaceRec_* face, byte* glyph_name)
        {
            return FTWindowsNative.FT_Get_Name_Index(face, glyph_name);
        }

        public FT_Err FT_Get_Glyph_Name(FT_FaceRec_* face, uint glyph_index, void* buffer, uint buffer_max)
        {
            return FTWindowsNative.FT_Get_Glyph_Name(face, glyph_index, buffer, buffer_max);
        }

        public byte* FT_Get_Postscript_Name(FT_FaceRec_* face)
        {
            return FTWindowsNative.FT_Get_Postscript_Name(face);
        }
        ...
        {
            return FTWindowsNative.FT_Get_SubGlyph_Info(glyph, sub_index, p_index, p_flags, p_arg1, p_arg2, p_transform);
        }
        ...
        {
            FTWindowsNative.FT_Vector_Transform(vector, matrix);
        }

        public void FT_Library_Version(FT_LibraryRec_* library, int* amajor, int* aminor, int* apatch)
        {
            FTWindowsNative.FT_Library_Version(library, amajor, aminor, apatch);
        }

        public byte FT_Face_CheckTrueTypePatents(FT_FaceRec_* face)
        {
            return FTWindowsNative.FT_Face_CheckTrueTypePatents(face);
        }

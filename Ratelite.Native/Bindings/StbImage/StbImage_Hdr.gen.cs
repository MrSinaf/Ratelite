...
{
	public static int stbi__hdr_test(StbiContext s)
	{
	...
	public static float* stbi__hdr_load(
		StbiContext s,
		int* x,
		int* y,
		int* comp,
	...
	)
	{
		...
		if (CRuntime.strcmp(headerToken, "#?RADIANCE") != 0 &&
			CRuntime.strcmp(headerToken, "#?RGBE") != 0)
			return (float*)(ulong)(Error("not HDR") != 0 ? 0 : 0);
		...
		if (valid == 0)
			return (float*)(ulong)(Error("unsupported format") != 0 ? 0 : 0);
		
		token = stbi__hdr_gettoken(s, buffer);
		if (CRuntime.strncmp(token, "-Y ", 3) != 0)
			return (float*)(ulong)(Error("unsupported data layout") != 0 ? 0 : 0);
		...
		if (height > 1 << 24)
			return (float*)(ulong)(Error("too large") != 0 ? 0 : 0);
		if (width > 1 << 24)
			return (float*)(ulong)(Error("too large") != 0 ? 0 : 0);
		...
		else
		{
			...
			for (j = 0; j < height; ++j)
			{
				...
				if (len != width)
				{
					...
					CRuntime.free(scanline);
					return (float*)(ulong)(Error("invalid decoded scanline length") != 0
							? 0
							: 0);
				}
					...
					while ((nleft = width - i) > 0)
					{
						...
						if (count > 128)
						{
							...
							if (count == 0 || count > nleft)
							{
								CRuntime.free(hdr_data);
								CRuntime.free(scanline);
								return (float*)(ulong)(Error("corrupt") != 0 ? 0 : 0);
							}
	...
	public static int stbi__hdr_info(StbiContext s, int* x, int* y, int* comp)
	{
		var buffer = stackalloc sbyte[1024];
		sbyte* token;
		var valid = 0;
	...
	public static int stbi__hdr_test_core(StbiContext s, string signature)
	{
		var i = 0;
	...
	public static sbyte* stbi__hdr_gettoken(StbiContext z, sbyte* buffer)
	{
		var len = 0;
		sbyte c = 0;
		c = (sbyte)Get8(z);
		while (AtEndOfStream(z) == 0 && c != 10)
		{
			buffer[len++] = c;
			if (len == 1024 - 1)
			{
				while (AtEndOfStream(z) == 0 && Get8(z) != 10) { }

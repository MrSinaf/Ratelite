...
internal unsafe partial class StbImage
{
	...
	public static void* stbi__png_load(
		StbiContext s,
		int* x,
		int* y,
		int* comp,
	...
	public static stbi__pngchunk stbi__get_chunk_header(StbiContext s)
	{
	...
	public static int stbi__check_png_header(StbiContext s)
	{
		var i = 0;
		for (i = 0; i < 8; ++i)
			if (Get8(s) != stbi__check_png_header_png_sig[i])
				return Error("bad png sig");
		...
		for (p = 0; p < 7; ++p)
		{
			...
			if (x != 0 && y != 0)
			{
				...
				for (i = 0; i < x; ++i)
				{
					...
					CRuntime.memcpy(
						final + out_y * a.s.imgX * out_bytes + out_x * out_bytes,
		...
		for (;;)
		{
			...
			switch (c.type)
			{
				...
				case ((uint)73 << 24) + ((uint)72 << 16) + ((uint)68 << 8) + 82:
				{
					...
					var filter = 0;
					if (first == 0)
						return Error("multiple IHDR");
					
					first = 0;
					if (c.length != 13)
						return Error("bad IHDR len");
					...
					if (z.depth != 1 && z.depth != 2 && z.depth != 4 && z.depth != 8 &&
						z.depth != 16)
						return Error("1/2/4/8/16-bit only");
					
					color = Get8(s);
					if (color > 6)
						return Error("bad ctype");
					...
					comp = Get8(s);
					if (comp != 0)
						return Error("bad comp method");
					
					filter = Get8(s);
					if (filter != 0)
						return Error("bad filter method");
					
					interlace = Get8(s);
					if (interlace > 1)
						return Error("bad interlace method");
					if (s.imgX == 0 || s.imgY == 0)
						return Error("0-pixel image");
				...
				{
					if (first != 0)
						return Error("first not IHDR");
					if (c.length > 256 * 3)
						return Error("invalid PLTE");
				...
				case ((uint)116 << 24) + ((uint)82 << 16) + ((uint)78 << 8) + 83:
				{
					...
					if (pal_img_n != 0)
					{
						...
						if (pal_len == 0)
							return Error("tRNS before PLTE");
						if (c.length > pal_len)
							return Error("bad tRNS len");
					...
					else
					{
						...
						if (z.depth == 16)
							...
							for (k = 0; k < s.imgN && k < 3; ++k)
								tc[k] = (byte)((byte)(stbi__get16be(s) & 255) *
				...
				case ((uint)73 << 24) + ((uint)69 << 16) + ((uint)78 << 8) + 68:
				{
					...
					if (scan != STBI__SCAN_load)
						return 1;
					if (z.idata == null)
						return Error("no IDAT");
					
					bpl = (uint)((s.imgX * z.depth + 7) / 8);
					raw_len = (uint)(bpl * s.imgY * s.imgN + s.imgY);
					z.expanded = (byte*)stbi_zlib_decode_malloc_guesssize_headerflag(
						(sbyte*)z.idata,
						(int)ioff,
						(int)raw_len,
						(int*)&raw_len,
					...
					z.idata = null;
					if ((req_comp == s.imgN + 1 && req_comp != 3 && pal_img_n == 0) ||
						has_trans != 0)
						s.imgOutR = s.imgN + 1;
					else
						s.imgOutR = s.imgN;
					...
					if (has_trans != 0)
					{
						if (z.depth == 16)
						{
							if (stbi__compute_transparency16(z, tc16, s.imgOutR) == 0)
								return 0;
						}
						else
						{
							if (stbi__compute_transparency(z, tc, s.imgOutR) == 0)
								return 0;
						}
					}
					...
					if (is_iphone != 0 &&
						(stbi__de_iphone_flag_set != 0
								...
								: stbi__de_iphone_flag_global) != 0 && s.imgOutR > 2)
						stbi__de_iphone(z);
					if (pal_img_n != 0)
					{
						s.imgN = pal_img_n;
						...
						if (req_comp >= 3)
							s.imgOutR = req_comp;
						if (stbi__expand_png_palette(z, palette, (int)pal_len, s.imgOutR) == 0)
							return 0;
					}
					else if (has_trans != 0)
					{
				...
				default:
					...
					if ((c.type & (1 << 29)) == 0)
					{
						...
						return Error(new string(parsePngFileInvalidChunk));
					}
	...
	)
	{
		...
		if (stbi__parse_png_file(p, STBI__SCAN_load, req_comp) != 0)
		{
			if (p.depth <= 8)
				...
				return (byte*)(ulong)(Error("bad bits_per_channel") != 0 ? 0 : 0);
			
			result = p._out_;
			p._out_ = null;
			if (req_comp != 0 && req_comp != p.s.imgOutR)
			{

...
internal unsafe partial class StbImage
{
	...
	public static int[] stbi__shiftsigned_mul_table =
	...
	public static int stbi__mad2sizes_valid(int a, int b, int add)
		=> stbi__mul2sizes_valid(a, b) != 0 && stbi__addsizes_valid(a * b, add) != 0 ? 1 : 0;
	
	public static int stbi__mad3sizes_valid(int a, int b, int c, int add)
		=> stbi__mul2sizes_valid(a, b) != 0 && stbi__mul2sizes_valid(a * b, c) != 0 &&
	...
	public static float* stbi__ldr_to_hdr(byte* data, int x, int y, int comp)
	{
		...
		{
			CRuntime.free(data);
			return (float*)(ulong)(Error("outofmem") != 0 ? 0 : 0);
		}
		...
		for (k = 0; k < n; ++k)
			output[i * comp + k] =
					(float)(CRuntime.pow(data[i * comp + k] / 255.0f, stbi__l2h_gamma) *
							stbi__l2h_scale);
	...
	)
	{
		...
		return (byte*)(ulong)(Error("unknown image type") != 0 ? 0 : 0);
	}
	...
	public static ushort* stbi__convert_8_to_16(byte* orig, int w, int h, int channels)
	{
		...
		if (enlarged == null)
			return (ushort*)(byte*)(ulong)(Error("outofmem") != 0 ? 0 : 0);
	...
	public static void stbi__vertical_flip(void* image, int w, int h, int bytes_per_pixel)
	{
		...
		for (row = 0; row < h >> 1; row++)
		{
			...
			{
				var bytes_copy =
						bytes_left < 2048 * sizeof(byte) ? bytes_left : 2048 * sizeof(byte);
	...
	public static void stbi__vertical_flip_slices(
		void* image,
		int w,
		int h,
		int z,
	...
	public static void stbi__float_postprocess(
		float* result,
		int* x,
		int* y,
		int* comp,
	...
	public static float* stbi__loadf_main(StbiContext s, int* x, int* y, int* comp, int req_comp)
	{
		byte* data;
		if (stbi__hdr_test(s) != 0)
		{
		...
		return (float*)(ulong)(Error("unknown image type") != 0 ? 0 : 0);
	}
	
	public static int stbi__get16be(StbiContext s)
	{
		int z = Get8(s);
		return (z << 8) + Get8(s);
	}
	
	public static uint stbi__get32be(StbiContext s)
	{
		var z = (uint)stbi__get16be(s);
		return (uint)((z << 16) + stbi__get16be(s));
	}
	
	public static int stbi__get16le(StbiContext s)
	{
		int z = Get8(s);
		return z + (Get8(s) << 8);
	}
	
	public static uint stbi__get32le(StbiContext s)
	{
		...
		for (j = 0; j < (int)y; ++j)
		{
			...
			switch (img_n * 8 + req_comp)
			{
				...
				default:
					...
					CRuntime.free(data);
					CRuntime.free(good);
					return (byte*)(ulong)(Error("unsupported") != 0 ? 0 : 0);
			}
		}
	...
	public static ushort stbi__compute_y_16(int r, int g, int b)
		=> (ushort)((r * 77 + g * 150 + 29 * b) >> 8);
	
	public static ushort* stbi__convert_format16(
		ushort* data,
		int img_n,
		int req_comp,
		uint x,
		...
		for (j = 0; j < (int)y; ++j)
		{
			...
			switch (img_n * 8 + req_comp)
			{
				...
				default:
					...
					CRuntime.free(data);
					CRuntime.free(good);
					return (ushort*)(byte*)(ulong)(Error("unsupported") != 0 ? 0 : 0);
			}
		}
	...
	public static int stbi__info_main(StbiContext s, int* x, int* y, int* comp)
	{
	...
	public static int stbi__is_16_main(StbiContext s)
	{

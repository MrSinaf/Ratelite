...
internal unsafe partial class StbImage
{
	...
	public static byte[] stbi__jpeg_dezigzag =
	{
		...
		20, 13, 6, 7,
		14, 21, 28, 35, 42, 49, 56, 57, 50, 43, 36, 29, 22, 15, 23, 30, 37, 44, 51, 58, 59, 52, 45,
		38, 31, 39, 46,
	...
	public static void* stbi__jpeg_load(
		StbiContext s,
		int* x,
		int* y,
		int* comp,
	...
	public static void stbi__grow_buffer_unsafe(stbi__jpeg j)
	{
		do
		{
			var b = (uint)(j.nomore != 0 ? 0 : Get8(j.s));
			if (b == 0xff)
			{
				int c = Get8(j.s);
				while (c == 0xff)
	...
	public static int stbi__process_marker(stbi__jpeg z, int m)
	{
				...
				while (L > 0)
				{
					...
					for (i = 0; i < 64; ++i)
						z.dequant[t][stbi__jpeg_dezigzag[i]] =
								(ushort)(sixteen != 0 ? stbi__get16be(z.s) : Get8(z.s));
					L -= sixteen != 0 ? 129 : 65;
				}
		...
		if ((m >= 0xE0 && m <= 0xEF) || m == 0xFE)
		{
			...
			{
				if (m == 0xFE)
					return Error("bad COM len");
				
				return Error("bad APP len");
			}
			...
			if (m == 0xE0 && L >= 5)
			{
				...
				var i = 0;
				for (i = 0; i < 5; ++i)
					if (Get8(z.s) != stbi__process_marker_tag[i])
						ok = 0;
				L -= 5;
				if (ok != 0)
					z.jfif = 1;
	...
	public static int stbi__process_scan_header(stbi__jpeg z)
	{
		...
		z.scan_n = Get8(z.s);
		if (z.scan_n < 1 || z.scan_n > 4 || z.scan_n > z.s.imgN)
			return Error("bad SOS component count");
		if (Ls != 6 + 2 * z.scan_n)
		...
		for (i = 0; i < z.scan_n; ++i)
		{
			...
			if (z.img_comp[which].hd > 3)
				return Error("bad DC huff");
			
			z.img_comp[which].ha = q & 15;
			if (z.img_comp[which].ha > 3)
				return Error("bad AC huff");
	...
	public static int stbi__process_frame_header(stbi__jpeg z, int scan)
	{
		...
		Lf = stbi__get16be(s);
		if (Lf < 11)
			return Error("bad SOF len");
		
		p = Get8(s);
		if (p != 8)
			return Error("only 8-bit");
		
		s.imgY = (uint)stbi__get16be(s);
		if (s.imgY == 0)
			return Error("no header height");
		
		s.imgX = (uint)stbi__get16be(s);
		if (s.imgX == 0)
			return Error("0 width");
		if (s.imgY > 1 << 24)
		...
		c = Get8(s);
		if (c != 3 && c != 1 && c != 4)
			return Error("bad component count");
		...
		for (i = 0; i < s.imgN; ++i)
		{
			...
			if (s.imgN == 3 && z.img_comp[i].id == stbi__process_frame_header_rgb[i])
				++z.rgb;
			q = Get8(s);
			z.img_comp[i].h = q >> 4;
		...
		z.img_mcu_w = h_max * 8;
		z.img_mcu_h = v_max * 8;
		z.img_mcu_x = (int)((s.imgX + z.img_mcu_w - 1) / z.img_mcu_w);
		z.img_mcu_y = (int)((s.imgY + z.img_mcu_h - 1) / z.img_mcu_h);
		for (i = 0; i < s.imgN; ++i)
		{
			z.img_comp[i].x = (int)((s.imgX * z.img_comp[i].h + h_max - 1) / h_max);
			z.img_comp[i].y = (int)((s.imgY * z.img_comp[i].v + v_max - 1) / v_max);
			...
			if (z.img_comp[i].raw_data == null)
				return stbi__free_jpeg_components(z, i + 1, Error("outofmem"));
	...
	{
		while (AtEndOfStream(j.s) == 0)
		{
			var x = Get8(j.s);
			while (x == 0xff)
			{
				if (AtEndOfStream(j.s) != 0)
					return 0xff;
	...
	public static int stbi__decode_jpeg_image(stbi__jpeg j)
	{
		...
		while (!(m == 0xd9))
			...
			else if (m == 0xdc)
			{
				...
				if (Ld != 4)
					return Error("bad DNL len");
				if (NL != j.s.imgY)
					return Error("bad DNL height");
	...
	public static byte* stbi__resample_row_v_2(
		byte* _out_,
		byte* in_near,
		byte* in_far,
		int w,
	...
	public static byte* stbi__resample_row_h_2(
		byte* _out_,
		byte* in_near,
		byte* in_far,
		int w,
	...
	public static byte* stbi__resample_row_hv_2(
		byte* _out_,
		byte* in_near,
		byte* in_far,
		int w,
	...
	public static byte* stbi__resample_row_generic(
		byte* _out_,
		byte* in_near,
		byte* in_far,
		int w,
	...
	public static void stbi__YCbCr_to_RGB_row(
		byte* _out_,
		byte* y,
		byte* pcb,
		byte* pcr,
	...
	public static byte* load_jpeg_image(
		stbi__jpeg z,
		int* out_x,
		int* out_y,
		int* comp,
	...
	)
	{
		...
		if (req_comp < 0 || req_comp > 4)
			return (byte*)(ulong)(Error("bad req_comp") != 0 ? 0 : 0);
		...
		n = req_comp != 0 ? req_comp : z.s.imgN >= 3 ? 3 : 1;
		is_rgb = z.s.imgN == 3 && (z.rgb == 3 || (z.app14_color_transform == 0 && z.jfif == 0))
				? 1
				: 0;
		...
		
		{
			...
			{
				var r = res_comp[k];
				z.img_comp[k].linebuf = (byte*)stbi__malloc(z.s.imgX + 3);
				...
				r.ystep = r.vs >> 1;
				r.w_lores = (int)((z.s.imgX + r.hs - 1) / r.hs);
				r.ypos = 0;
			...
			for (j = 0; j < z.s.imgY; ++j)
			{
				...
				else
				{
					if (is_rgb != 0)
					{
						if (n == 1)
							for (i = 0; i < z.s.imgX; ++i)
								*_out_++ = stbi__compute_y(
									coutput[0][i],
									coutput[1][i],
									coutput[2][i]
								);
						else
							for (i = 0; i < z.s.imgX; ++i, _out_ += 2)
							{
								_out_[0] = stbi__compute_y(
									coutput[0][i],
									coutput[1][i],
									coutput[2][i]
								);
					...
					else if (z.s.imgN == 4 && z.app14_color_transform == 0)
					{
						for (i = 0; i < z.s.imgX; ++i)
						{
							var m = coutput[3][i];
					...
					else if (z.s.imgN == 4 && z.app14_color_transform == 2)
					{
						for (i = 0; i < z.s.imgX; ++i)
						{

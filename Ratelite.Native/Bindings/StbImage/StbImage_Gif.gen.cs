...
internal unsafe partial class StbImage
{
	...
	public static void* stbi__gif_load(
		StbiContext s,
		int* x,
		int* y,
		int* comp,
				...
				if (u != null)
				{
					...
					*y = g.h;
					...
					if (_out_ != null)
					{
						...
						if (delays != null)
						{
							var new_delays = (int*)CRuntime.realloc(
								*delays,
								(ulong)(sizeof(int) * layers)
							);
							if (new_delays == null)
								return stbi__load_gif_main_outofmem(g, _out_, delays);
	...
	public static int stbi__gif_header(StbiContext s, stbi__gif g, int* comp, int is_info)
	{
		byte version = 0;
		if (Get8(s) != 71 || Get8(s) != 73 || Get8(s) != 70 ||
			Get8(s) != 56)
			return Error("not GIF");
	...
	public static byte* stbi__process_gif_raster(StbiContext s, stbi__gif g)
	{
		byte lzw_cs = 0;
		var len = 0;
		var init_code = 0;
		...
		for (;;)
			...
			else
			{
				...
				{
					if (first != 0)
						return (byte*)(ulong)(Error("no clear code") != 0 ? 0 : 0);
					...
					if (oldcode >= 0)
					{
						fixed (stbi__gif_lzw* p2 = &g.codes[avail++])
						{
							p = p2;
							if (avail > 8192)
								return (byte*)(ulong)(Error("too many codes") != 0 ? 0 : 0);
					...
					{
						return (byte*)(ulong)(Error("illegal code in raster") != 0 ? 0 : 0);
					}
		...
		for (;;)
		{
			...
			switch (tag)
			{
				case 0x2C:
				{
					...
					h = stbi__get16le(s);
					if (x + w > g.w || y + h > g.h)
						return (byte*)(ulong)(Error("bad Image Descriptor") != 0 ? 0 : 0);
					...
					{
						return (byte*)(ulong)(Error("missing color table") != 0 ? 0 : 0);
					}
				...
				case 0x3B:
					return null;
				default:
					return (byte*)(ulong)(Error("unknown code") != 0 ? 0 : 0);
			}
		}
	}

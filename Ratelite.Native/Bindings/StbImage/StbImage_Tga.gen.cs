...
{
	public static int stbi__tga_test(StbiContext s)
	{
		var res = 0;
		var sz = 0;
	...
	public static void* stbi__tga_load(
		StbiContext s,
		int* x,
		int* y,
		int* comp,
	...
	)
	{
		...
		if (tga_indexed != 0)
			...
			tga_comp = stbi__tga_get_comp(
				tga_bits_per_pixel,
				tga_image_type == 3 ? 1 : 0,
				&tga_rgb16
			);
		if (tga_comp == 0)
			return (byte*)(ulong)(Error("bad format") != 0 ? 0 : 0);
		...
		else
		{
			if (tga_indexed != 0)
			{
				if (tga_palette_len == 0)
				{
					...
					return (byte*)(ulong)(Error("bad palette") != 0 ? 0 : 0);
				}
			...
			for (i = 0; i < tga_width * tga_height; ++i)
			{
				...
				if (read_next_pixel != 0)
				{
					if (tga_indexed != 0)
					{
						var pal_idx = tga_bits_per_pixel == 8 ? Get8(s) : stbi__get16le(s);
	...
	public static int stbi__tga_info(StbiContext s, int* x, int* y, int* comp)
	{
		var tga_w = 0;
		var tga_h = 0;
		var tga_comp = 0;
	...
	public static void stbi__tga_read_rgb16(StbiContext s, byte* _out_)
	{
